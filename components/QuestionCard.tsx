...
export default function QuestionCard({ question, onNext }: { question: Question; onNext?: () => void }) {
    const { data: session, update } = useSession()
    const [selectedOption, setSelectedOption] = useState<number | null>(null)
    const [submitted, setSubmitted] = useState(false)
    const [result, setResult] = useState<{ isCorrect: boolean; correctOption: number; explanation: string | null } | null>(null)
    ...
    return (
        <div className="w-full max-w-md bg-[#111] rounded-xl shadow-2xl border border-gray-800 overflow-hidden flex flex-col max-h-[80vh]">
            <div className="p-4 sm:p-6 overflow-y-auto custom-scrollbar">
              ...
              ${question.difficulty === 'EASY' ? 'bg-green-900/20 text-green-400 border-green-900' :
                                question.difficulty === 'MEDIUM' ? 'bg-yellow-900/20 text-yellow-400 border-yellow-900' :
                                    'bg-red-900/20 text-red-400 border-red-900'}`}>
                            {question.difficulty}
                        </span>
                        <h3 className="text-xl font-bold text-gray-100 font-mono">{question.title}</h3>
                    </div>
                    <span className="text-xs text-gray-400 bg-gray-900 border border-gray-800 px-2 py-1 rounded whitespace-nowrap ml-2 font-mono">{question.category}</span>
                </div>

                <p className="text-gray-300 mb-6 whitespace-pre-wrap font-mono text-sm leading-relaxed">{question.description}</p>
                ...
                <div className="space-y-3">
                    ...
                    ${selectedOption === index ? 'border-green-500' : 'border-gray-600'}`}>
                                        {selectedOption === index && <div className="w-3 h-3 rounded-full bg-green-500" />}
                                    </div>
                                    ...
                                    {submitted && result && index === selectedOption && !result.isCorrect && (
                    ...
                    <div className="mt-6 space-y-3">
                        ...
                        <AnimatePresence>
                            {showExplanation && result?.explanation && (
                                ...
                                >
                                    <h4 className="font-bold mb-2 text-green-500">Explanation:</h4>
