...
export default function BulkUpload() {
    ...
    const validateQuestion = (q: any) => {
        if (!q.title || typeof q.title !== 'string') throw new Error("Missing or invalid title")
        if (!Array.isArray(q.options) || q.options.length < 2) throw new Error(`Invalid options for question: ${q.title}`)
        if (typeof q.correctOption !== 'number' && typeof q.correctOption !== 'string') throw new Error(`Invalid correctOption for question: ${q.title}`)
        return true
    }
        ...
        try {
            ...
            if (file.name.endsWith('.json')) {
                ...
                formattedQuestions = jsonData.map((row: any) => ({
                    title: row.title.trim(),
                    description: (row.description || "").trim(),
                    options: typeof row.options === 'string' ? row.options.split('|') : row.options,
