        ...
        <nav className="bg-[#0a0a0a] border-b border-gray-800 sticky top-0 z-50 backdrop-blur-sm bg-opacity-80">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-16">
                    <div className="flex">
                        <Link href="/" className="flex-shrink-0 flex items-center font-bold text-xl text-green-500 tracking-tighter font-mono">
                            &lt;LeetScroll /&gt;
                        </Link>
                        <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
                            <Link href="/" className="border-transparent text-gray-400 hover:text-green-400 hover:border-green-500 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors font-mono">
                                Feed
                            </Link>
                            <Link href="/leaderboard" className="border-transparent text-gray-400 hover:text-green-400 hover:border-green-500 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors font-mono">
                                Leaderboard
                            </Link>
                        </div>
                    </div>
                        ...
                        {session ? (
                            <div className="flex items-center space-x-4">
                                <span className="text-sm text-gray-400 font-mono">
                                    Score: <span className="font-bold text-green-500">{session.user.score || 0}</span>
                                </span>
                                {session.user.role === "ADMIN" && (
                                    <Link href="/admin" className="text-sm text-gray-400 hover:text-white transition-colors font-mono">
                                        Admin
                                    </Link>
                                )}
                                <button
                            ...
                            <div className="space-x-4">
                                <Link href="/login" className="text-gray-400 hover:text-white text-sm font-medium transition-colors font-mono">
                                    Log in
                                </Link>
                                <Link href="/register" className="bg-green-600 text-black px-4 py-2 rounded-md text-sm font-bold hover:bg-green-500 transition-colors font-mono">
                                    Sign up
                                </Link>
                            </div>
                        )}
                        ...
                        {session ? (
                            <div className="space-y-1">
                                <div className="px-4 flex items-center justify-between">
                                    <span className="text-base font-medium text-gray-300 font-mono">{session.user.name}</span>
                                    <span className="text-sm text-gray-400 font-mono">
                                        Score: <span className="font-bold text-green-500">{session.user.score || 0}</span>
                                    </span>
                                </div>
